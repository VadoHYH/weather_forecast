class WeatherModel {
    ...
    getFullLocationName() {
      return this.selectedCity + (this.selectedDistrict ? ` ${this.selectedDistrict}` : '');
    }
    ...
    getWeatherData() {
      ...
      this.currentImageKey = `${tempCategory}_${weatherCondition}`;
      this.currentImageIndex = 1; // 重置圖片編號
      ...
      return {
        ...
        advice: this.adviceMap[this.currentImageKey] || '今天天氣變化多端，請小心～'
      };
    }
  
    // 獲取下一張圖片的索引
    getNextImageIndex() {
      const totalImages = this.imageCounts[this.currentImageKey] || 1;
      this.currentImageIndex = this.currentImageIndex % totalImages + 1;
      return this.currentImageIndex;
    }
  
    // 獲取當前圖片路徑
    getCurrentImagePath() {
      return `./img_V/${this.currentImageKey}_${this.currentImageIndex}.svg`;
    }
}
